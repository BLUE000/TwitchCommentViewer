...
### 3.1 暗号化・難読化
* プライバシー保護およびセキュリティの観点から、DBのみならずシステムが生成する中間ファイル（ログの一時キャッシュ、設定ファイル、集計用の一時エクスポート等）について、`TransCipher-Dist` を利用して難読化（暗号化）して保存します。
...
* Windows以外のOSでは、`TransCipher-Dist` による共通鍵暗号化方式にフォールバックします。
