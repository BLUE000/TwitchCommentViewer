...
* **メイン領域**: `QTabWidget`（以下の3タブ構成）
  ...
  2. **解析タブ**: `QTextBrowser` またはグラフ用Widget（トレンド・スパム等の集計表示）
...
* **画面構成 (DBViewerWindow.ui)**:
  * **ファイル選択**: 閲覧するDBファイルを選択するボタン。
  * **データ表示**: `QTableView` を用い、暗号化された `username_enc` や `message_enc` をメモリ上で復号して平文として一覧表示する。
  * **検索・抽出**: 特定のユーザーIDやキーワードでログを検索・フィルタリングする機能。
